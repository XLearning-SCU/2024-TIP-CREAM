...
def evalrank(model_path, data_path=None, vocab_path=None, split="dev", fold5=False):
    ...
    if not fold5:
        ...
        print("Average i2t Recall: %.2f" % mean_i2t)
        print("Image to text: %.2f %.2f %.2f %.2f %.2f" % mean_metrics[:5])
        mean_t2i = (mean_metrics[5] + mean_metrics[6] + mean_metrics[7]) / 3
        print("Average t2i Recall: %.2f" % mean_t2i)
        print("Text to image: %.2f %.2f %.2f %.2f %.2f" % mean_metrics[5:10])

...
class SamplesDataSet(data.Dataset):
    ...
    def __init__(
        ...
        # MS-COCO have 25K pairs for validation, but only 5K is used.
        if data_split == "dev":
        ...
        if data_split == "train":
            ...
            if noise_ratio:
                if os.path.exists(noise_file):
                    # If it is the first training epoch (including warm-up), 
                    # then randomly disrupt the {noise_ratio}% data and save the index. 
                    # If it is not the first training epoch (including warm-up), 
                    # the previously saved index file is read.
            ...
            if self.mode == "clean" or self.mode == "hard":
